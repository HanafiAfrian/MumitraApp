...
export default class SudahSampai extends Component {
	constructor(props) {
    super(props);
    this.state = {
		   nama: '',
 ...
 klikSimpan() {
     if (
      ...
      if (this.state.idEdit) {
        ...
        var urlAksi = this.url + '/?op=updateselesai&invoice=' + invoice;
      } else {
		  var invoice =	this.props.route.params.invoice; 
       var urlAksi = this.url + '/?op=updateselesai&invoice=' + invoice;
  	   var invoice =	this.props.route.params.invoice; 
    ...
    setTimeout(() => {
		alert ('Pesanan Telah Selesai di Kerjakan')
  ...
  <Text style={styles.resto}> {nama_produk}</Text>
   ...
   pinColor={"#0000ff"}/>
       ...
       onPress={() => this.klikSimpan()}>
            <Text style={styles.btntext}> Pesanan Selesai</Text>
          </TouchableOpacity>
		    <TouchableOpacity
